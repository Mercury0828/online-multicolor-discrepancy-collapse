...
\]
The discrepancy of a coloring is $\max_{t\le T,\ i\in[n]}\rangeop_i(t)$, the largest pairwise color-load imbalance
over all prefixes and coordinates. We study its optimal value:
\[
  \ondisc_k(\mu_{d,n})\ :=\ \min_{\text{online }\chi}\ \max_{t,i}\rangeop_i(t),
  \qquad
  \offdisc_k(\mu_{d,n})\ :=\ \min_{\text{offline }\chi}\ \max_{t,i}\rangeop_i(t),
\]
each understood \whp\ (and, where stated, in expectation). We abbreviate $\ondisc_k=\ondisc_k(\mu_{d,n})$ and
$\offdisc_k=\offdisc_k(\mu_{d,n})$. Since every online coloring is also an offline coloring,
$\ondisc_k\ge\offdisc_k$ always.
