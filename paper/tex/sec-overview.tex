...
that three properties of this rule are \emph{independent of the number of colors}: a minimum-load assignment never raises the
range of the coordinate it repairs (which is exceptional, hence of positive range, for any $k$), the assigned color differs
from the seed only when the arrival already contains an exceptional coordinate, and an exceptional coordinate's range rises
only with a co-present helper of at least its range. We package these into a self-contained transfer lemma (\Cref{lem:transfer})
that proves $\max_{t,i}\rangeop_i=\Oh(\loglog n)$ for any rule meeting the interface, by a stopped contact count for the
exceptional set and a double-exponential level recursion; the number of colors enters nowhere. The one genuinely
$k$-dependent estimate is the seed tail, the probability that a random $k$-coloring leaves a coordinate exceptional, which we
bound by a maximal Bernstein inequality whose variance $2Td/(nk)$ shrinks as $k$ grows; a two-regime calculation gives the
bound for all $k\le d$ and all fixed $k$, so the upper bound $\Oh(\loglog n)$ holds throughout. The lower bound
$\Om(\loglog n)$ for fixed $k$ projects the colors onto two by a zero-sum signing and applies a self-contained two-action
lower bound (\Cref{lem:asym-online}); for odd $k$ the signing is asymmetric, with steps in $\{+1,-\rho\}$, $\rho\in[\tfrac12,1]$.
...
permutation-invariant potential loses a $\log k$ factor and so cannot reach it: a phase-conveyor construction refutes the
expectation-level tail that such a potential would need. The obstruction is the same staggered-Fibonacci mechanism that separates \emph{Always-Go-Left}
