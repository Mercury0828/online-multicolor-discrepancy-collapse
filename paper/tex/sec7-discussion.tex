...
\Oh(\sqrt d)$, and the two large-deviation terms are at most $\log k\le\sqrt k\le\sqrt d$ in the stated range (using $k\le d$); so
...
At $k=2$ the offline value is the classical $\Th(\sqrt d)$ of a random sparse set system
\cite{BansalMeka2019,EzraLovett2016,HobergRothvoss2019} (our \Cref{thm:separation} uses only the upper half, $\Oh(\sqrt d)$);
so from constant $k$ to $k=\Th(d)$ the offline discrepancy ranges from $\Th(\sqrt d)$ down to the balls-into-bins value
