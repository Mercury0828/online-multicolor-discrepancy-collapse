...
fixed $k\ge 2$ & $\Th(\sqrt d)$ & $\Th(\loglog n)$ & \Cref{thm:separation} \\
$\log^5(dk)\le k\le d$ & $\Th(\Psifn(\lam,\log k))$ & $[\max\{\Psifn,\tfrac{\loglog n}{k}\},\,\loglog n]$ & \Cref{thm:offline}, \Cref{cor:bracket} \\
\hline
\end{tabular}
\caption{The offline value collapses to the occupancy threshold $\Psifn(\lam,\log k)$ as $k$ grows, while the online value
stays at the $\Th(\loglog n)$ barrier for every fixed $k$. The growing-$k$ online order is bracketed
(\Cref{sec:open}).}\label{tab:results}
\end{table}
...
A line of constructive algorithms now matches these existentially: Bansal's SDP rounding \cite{Bansal2010}, the Lovett--Meka
partial-coloring walk \cite{LovettMeka2015}, Rothvoss's convex-set method \cite{Rothvoss2017}, and the Gram--Schmidt walk and
its relatives \cite{BansalGarg2017,BansalDadushGargLovett2018}. For random set systems the picture is sharper than the
worst case: the discrepancy of a random $d$-regular system is $\Th(\sqrt d)$, due to Bansal--Meka, Ezra--Lovett, and
Hoberg--Rothvoss \cite{BansalMeka2019,EzraLovett2016,HobergRothvoss2019}. We use \cite{LovettMeka2015} as our splitting
primitive.
...
Our occupancy threshold $\Psifn$ is exactly the maximum-load law of (weighted) balls-into-bins allocation
\cite{ABKU1999,BCSV2006,RaabSteger1998,TalwarWieder2007}, and the growing-$k$ obstruction in \Cref{sec:open} is the same
staggered-Fibonacci mechanism behind \emph{Always-Go-Left} \cite{Vocking2003,MitzenmacherUpfal2017}.

\paragraph{Tools.}
Beyond \cite{LovettMeka2015}, our offline construction rests on negative association and strong Rayleigh measures
\cite{JoagDevProschan1983,DubhashiRanjan1998,Pemantle2000,BorceaBrandenLiggett2009,BrandenHuh2020} and on Bennett-type Poisson
tails \cite{Bennett1962,BoucheronLugosiMassart2013}; the recursive exact-cardinality rounding parallels online dependent
rounding for matchings \cite{NSW2025}.
