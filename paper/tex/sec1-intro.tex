...
\begin{itemize}
  \item[\textbf{(T1)}] (\Cref{thm:offline}) For $C_0\log^5(dk)\le k\le d$ with $d,k=(\log n)^{o(1)}$, the optimal offline
...
($\log^5(dk)\le k\le d$), $\max\{\Psifn(\lam,\log k),\loglog n/k\}\le\ondisc_k\le\loglog n$. The online-specific term $\loglog n/k$ exceeds the offline floor for small $k$, while the floor
