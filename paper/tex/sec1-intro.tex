...
$\log^5(dk)\le k\le d,\ \lam\ge1$ & $\Th(\Psifn(\lam,\log k))$ & $[\max\{\Psifn(\lam,\log k),\tfrac{\loglog n}{k}\},\,\loglog n]$ & \Cref{thm:offline}, \Cref{cor:bracket} \\
\hline
\end{tabular}
...
($\log^5(dk)\le k\le d$, $\lam\ge1$), $\max\{\Psifn(\lam,\log k),\loglog n/k\}\le\ondisc_k\le\loglog n$. The online-specific
