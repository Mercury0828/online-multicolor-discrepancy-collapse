...
$k$, and the upper bound $\Oh(\loglog n)$ in fact holds for all $k\le d$ and for every fixed $k$.
...
term $\loglog n/k$ exceeds the offline floor for small $k$, while the floor $\Psifn$ takes over as $k$ approaches $d$. We
identify a sufficient high-probability lemma about an asymmetric online rule whose truth would pin the order, and show that a
natural symmetric, permutation-invariant potential loses a $\log k$ factor (\Cref{sec:open}). We conjecture
...
The crux is producing, from a single draw of the incidence matrix, one $k$-coloring whose every coordinate
...
that (positive-range) coordinate's range. We isolate a three-property interface on the repair rule under which the category
and double-exponential machinery of the two-color analysis applies in the range language, with the number of colors entering
nowhere; we state the interface and verify our algorithm meets it in \Cref{app:online}.
